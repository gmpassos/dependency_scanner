...
class DependencyScanner {
  ...
  Future<List<String>> getScannedProjectsNames() async {
    return await Future.wait( _scannedProjects.map( (p) async => (await p.pubSpec).name ).toList() ) ;
  }
  ...
  Future< List<Project> > scan() async {
    ...
    for (var project in projects) {
      ...
      if ( lastVersion != null && pubSpec.version > lastVersion && versions.isNotEmpty) {
  ...
  Future<bool> doCommand( String command , List<String> args ) async {
    var commandSimple = command.toLowerCase().trim().replaceAll(RegExp(r'[\W_]'), '') ;

    consoleLine();
    print('EXECUTING COMMAND: $command $args') ;

    switch (commandSimple) {
      case 'upgradedependency': return doCommand_upgradeDependency(args) ;
      case 'pubget': return doCommand_pubGet(args) ;
      case 'localpath': return doCommand_localPath(args) ;
      case 'rollbacklocalpath': return doCommand_rollback_localPath(args) ;
      default: {
  ...
  Future<bool> doCommand_upgradeDependency(List<String> packages) async {
    ...
    var dependenciesVersions = packages.map( (p) => DependencyVersion.parse(p) ).toList() ;
    ...
    for ( var project in _scannedProjects ) {
      ...
      if ( !hasGit ) {
        print("    ** Project '${pubSpec.name}' doesn't have Git! Can't make safe modifications: skipping: ${ project.directory }\n") ;
        continue ;
      }
      ...
      for (var entry in pubSpec.dependencies.entries) {
        ...
        var dependenciesVersion = DependencyVersion.getByPackage(dependenciesVersions, depName) ;
        ...
        if (targetVersion == null) {
          ...
          if ( !packVersions.contains(targetVersion) ) {
            throw StateError("Can't upgrade package to version: $depName $targetVersion > published versions: $packVersions");
          }
        }
        ...
        if (depVerRef is HostedReference) {
          ...
          if ( depVer < targetVersion ) {
            var targetVersionStr = verUpper ? '^$targetVersion' : '$targetVersion' ;
            print('    - Dependency: $depName ${ verUpper ? '^' : ''}$depVer -> $targetVersionStr');

            await _rewritePubSpec_dependencyVersion(project, depName, targetVersionStr);
          }
        }
      }
  ...
  Future<bool> _rewritePubSpec_dependencyVersion(Project project, String depName, String targetVersion) async {
    var pubSpec = await project.pubSpec ;
    ...
    var pattern = regExpDialect(
        dialect,
        r'($n$s*dependencies:$s*$dep*?)($depTarget)($pathComm)?',
        multiLine: true
    ) ;
  ...
  Future<bool> doCommand_pubGet(List<String> projects) async {
    ...
    var all = projects.firstWhere( (p) => p == '*' || p.toLowerCase() == 'all' , orElse: () => null ) != null ;
      ...
      for (var entry in pubSpec.dependencies.entries) {
        ...
        if ( depVerRef is HostedReference ) {
          ...
          if ( localPubSpec.version < depVer ) {
            print('    ** Local project in a older version: ${ localPubSpec.name }:${ localPubSpec.version } < $depName:$verStr ') ;
            continue ;
          }
          else if ( localPubSpec.version > depVer && !verUpper ) {
            print('    ** Local project in a upper version: ${ localPubSpec.name }:${ localPubSpec.version } > $depName:$verStr ') ;
            continue ;
          }

          print('    - $depName[$verStr] -> [${ localPubSpec.version }] ${ localProject.directory.path }');


          await _rewritePubSpec_dependencyPath(project, depName, localProject.directory.path) ;
        }
      }
  ...
  Future<bool> _rewritePubSpec_dependencyPath(Project project, String depName, String localPath) async {
    ...
    var data2 = regExpReplaceAllMapped(pattern, data, (match) {
      ...
      if (g3.isNotEmpty) {
        var patternPath = regExpDialect(
            dialect,
            '^(\$n\$s+)#(path:\$s*)($localPath|$localPathOrig)(\$s*)',
            multiLine: true
        ) ;
    ...
    for (var projectName in projects) {
      ...
      for (var entry in pubSpec.dependencies.entries) {
        ...
        if ( depVerRef is PathReference ) {
          ...
          print('    - $depName[$depPath] -- ROLLBACK [${ localPubSpec.version }] ${ localProject.directory.path }');

          await _rewritePubSpec_dependencyHosted(project, depName, localProject.directory.path) ;
        }
      }
  ...
  Future<bool> _rewritePubSpec_dependencyHosted(Project project, String depName, String localPath) async {
    var localPathOrig = localPath ;
    ...
    var pattern = regExpDialect(
        dialect,
        r'($n$s*dependencies:$s*$dep*?)($depTarget)($path)?',
        multiLine: true
    ) ;
  ...
  void _checkPubSpec(PubSpec pubSpec, PubSpec pubSpec2, String ignoreDependency) {
    if ( pubSpec.name != pubSpec2.name ) throw StateError('name') ;
    if ( pubSpec.version != pubSpec2.version ) throw StateError('version') ;
    if ( pubSpec.description != pubSpec2.description ) throw StateError('description') ;
    if ( pubSpec.documentation != pubSpec2.documentation ) throw StateError('documentation') ;
    ...
    if ( !isEqualsAsString( pubSpec.environment , pubSpec2.environment ) ) throw StateError('environment') ;
    if ( !isEqualsDeep( pubSpec.dependencyOverrides , pubSpec2.dependencyOverrides ) ) throw StateError('dependencyOverrides') ;
  ...
  String _pathReference_toString(PathReference depVerRef) {
    var s = depVerRef.toString();
    return s.replaceAll(RegExp(r'(^"|"$)'), '');
  }
...
class DependencyVersion {
  ...
  static DependencyVersion getByPackage(List<DependencyVersion> list, String name) {
  ...
  DependencyVersion(this.name, [dynamic version]) :
        version = version is Version ? version : ( version is String || version is num ? Version.parse('$version'.trim()) : null )
  {
    if ( !RegExp(r'^\w+$').hasMatch(name) ) throw ArgumentError('Invalid name: $name');
  }
